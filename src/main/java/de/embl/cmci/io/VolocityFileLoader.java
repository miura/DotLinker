...
public class VolocityFileLoader implements IDataLoader {
...
//			Particle particle = new Particle(xA[i], yA[i], zA[i], (int) (timeA[i] - 1), i);
			Particle particle = dotlinker.new Particle(xA[i], yA[i], zA[i], sxA[i], syA[i], szA[i], (int) (timeA[i] - 1), i);
...
//			IJ.log("track length " + i + ": " + counter[i]);
	 ...
	 *  depricated
	 */
	public StackFrames[] dataloaderOLD(DotLinker dotlinker){
...
//		int endframe = (int) sliceAsort[sliceAsort.length-1];
		...
		// fill in the Myframe object
		for (int i = 0 ; i< timeA.length; i++){
			Particle particle = dotlinker.new Particle(xA[i], yA[i], zA[i], (int) (timeA[i] - 1), i);
			frameA[particle.getFrame()].getParticles().add(particle);
		}
		return frameA;
	}	
